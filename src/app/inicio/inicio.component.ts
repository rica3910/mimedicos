...
export class InicioComponent implements OnInit {
  ...
  public reporte() {
    //Imagen o logo que aparecerá en el encabezado del pdf.
    ...
    let infoEncabezado: Object = {
      imagen: imagen,
      organizacion: "CENTRO VASCULAR",
      direccion: "MENDEZ 3910, Col. Obrera, CP.3115",      
      estado: "Chihuahua, Chihuahua, Mex.",
      telefono: "6144158665"
    }

    let infoPiePagina: Object = {    
      imagen: imagen,  
      nombre: "RICARDO SALVADOR LUNA LOZOYA",
    }    
    ...
    let lineas: Array<string> = this.pdfcartaService.pdf.splitTextToSize(textit, this.pdfcartaService.getPosicionContenido()["fin"]["x"]);
    let posicionYActual: number = this.pdfcartaService.getPosicionContenido()["inicio"]["y"];

    lineas.forEach(linea => {

      this.pdfcartaService.pdf.text(linea, this.pdfcartaService.getPosicionContenido()["inicio"]["x"], posicionYActual);
      posicionYActual = posicionYActual + this.pdfcartaService.getEspacioEntreRenglones();
      
      if(posicionYActual >= this.pdfcartaService.getPosicionContenido()["fin"]["y"]){
        posicionYActual = this.pdfcartaService.getPosicionContenido()["inicio"]["y"];
        this.pdfcartaService.pdf.addPage();
      }
    ...
    this.pdfcartaService.pdf.save('ejemplo.pdf');
